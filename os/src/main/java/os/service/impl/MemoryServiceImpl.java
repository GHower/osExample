...
public class MemoryServiceImpl implements MemoryService {
    ...
    public MyPCB allocation(MyPCB myPCB, MyProcess myProcess) {
        ...
        idleLinked = idleLinked.stream()
                .sorted(Comparator.comparingInt(MyMemory::getSize))
...
//        display();
    ...
    public boolean remove(MyPCB myPCB) {
        ...
        pcbPool = pcbPool.stream().filter(e-> !e.getPid().equals(myPCB.getPid())).collect(Collectors.toList());
            ...
            if (myPCB.getAddr() < cur.getAddress()) {
                ...
                if (pre == null) {
                    ...
                    if (preEnd != delStart && curAddr != delEnd ) {
                        ...
                        // 3. 三点相连
                        cur.setSize(pre.getSize() + cur.getSize() + myPCB.getSize());
                        cur.setAddress(pre.getAddress());
                        idleLinked.set(i - 1, cur);
                        idleLinked.remove(i);
